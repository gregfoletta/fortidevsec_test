...
resource "aws_network_interface" "interfaces" {
    ...
    private_ip_list_enabled = can( each.value["ipv4_index"])
    private_ip_list = can( each.value["ipv4_index"]) ? [ cidrhost( var.site_subnets[ each.value.subnet ].cidr_block, each.value.ipv4_index) ] : [ ]
